...
al.'s [*Learning Analytics Goes to School*](library(t)*.* You can check
that out, but don't feel any need to dive deep for now - we'll be
spending more time on this in first day of the summer institute. For
now, know that this document is organized around three of the five
...
-   Grade category has missing values

-   Wonder about the unit of analysis, student-term-course-section?
...
-   Student's course taking, student-term-course (section) level data

What columns typically (or you think may) represent:

-   Variables that measure students' performance and effort on courses
    taken per semester
...
It filters the obs with more than 80% in final gradesCEMS
...
```{r}d |> select(student_id, points_earned, gender) |>filter(total_points_earned>2000)}
...
```{r}
ggplot(d, aes(x = total_points_earned, y = percentage_earned)) +
  geom_point()
```
